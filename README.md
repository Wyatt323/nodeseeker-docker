...
| 🧩 布尔关键词规则 | 支持独立关键词、显式 `AND`、括号内 `OR` 组合匹配 |
| 📨 富文本帖子推送 | 标题、命中规则、原帖链接和 NodeSeek 网页预览卡片 |
...
Bot 启动时会自动注册以下命令到 Telegram 命令菜单；发送 `/commands` 或 `/help` 也会返回完整功能说明。
...
| `/list` | 按规则列出自己的关键词；每条规则使用 Telegram 代码格式，点击即可复制，不显示内部 ID |
| `/add 关键词1 关键词2 关键词3` | 分别添加多条独立规则，匹配任意一个就会推送 |
| `/add 关键词1 AND 关键词2` | 添加一条 AND 组合规则，所有关键词都要匹配 |
...
# 示例
/add 你好 我好 大家好
# 分别添加“你好”“我好”“大家好”三条独立规则，任意一个匹配就会推送

/add 你好 AND 我好 AND 大家好
# 添加一条组合规则，三个关键词必须同时匹配才会推送
...
匹配成功后，Bot 使用 Telegram HTML 富文本发送消息：

```text
🔔 标题：帖子标题

📌 匹配：重置 AND (chatgpt OR gpt OR codex)

🔗 点击查看原帖 →
```

- “标题”和“匹配”只使用粗体，不使用下划线。
- “匹配”显示完整的人类可读规则，不会暴露数据库 ID 或内部 `or:` 编码。
- “点击查看原帖”是可点击链接。
- Telegram 会在消息底部自动生成 NodeSeek 网页预览卡片，并优先使用右侧小缩略图布局。
- 标题和规则会进行 HTML 转义，避免 RSS 内容破坏消息格式。
- 网页预览最终样式由 Telegram 客户端和 NodeSeek 页面元数据决定，不同客户端可能略有差异。
